...
///
/// You can change cursor next and previus with methods which always guarantee  range bounds.
///
/// Since cursorvec redirects deref trait to inner container, std lib's vector methods are also applicable to cursorvec. However cursor will not be automatically updated so that cursor is in range bounds. In such case call "update_cursor" manually after or bind the methods inside cursorvec's "modify" method.
///
/// # Usage
///
/// ```rust
